...
/**
 * regex to find emojis in a string - finds emoji, e.g. 💯
 */
var searcher = (function() {
  var values = [];
  for (var prop in Emoji.emoji) {
    if (Emoji.emoji.hasOwnProperty(prop)) {
      var emoji = Emoji.emoji[prop];
      var charCodes = [];
      for (var n = 0; n < emoji.length; n++) {
        charCodes.push(emoji.charCodeAt(n));
      }

      var value = charCodes.map(function(codepoint) {
  ...
  // Sort desc by length in case some emojis are substrings of others
...
/**
 * unemojify a string (replace all emojis with :emoji:)
 * @param {string} str
 * @return {string}
 */
Emoji.unemojify = function unemojify(str) {
  ...
  return str.split(searcher) // search emoji via regex
            .map(function parseEmoji(s, i) {
              // every second element is an emoji, e.g. "test 💯" -> [ "test ", "💯" ]
              if (i % 2 === 0) return s;
              return ':' + Emoji.which(s) + ':';
            })
            .join('') // convert back to string
  ;
}
